...
def main() -> None:
    # Initialize parser
    parser = argparse.ArgumentParser(description="Generate an interactive property map from Nashville housing data")

    # Get database config from environment
    db_config = get_db_config()
    
    # Adding arguments with environment defaults
    parser.add_argument("--hostname", default=db_config["hostname"],
    ...
    parser.add_argument('--output', default="nashville_property_map.html",
...
def create_property_map(args: argparse.Namespace) -> None:
    ...
    engine = create_engine(
        f'postgresql://{args.username}:{password}@{args.hostname}:{args.port}/{args.database}'
    )
    ...
    try:
        with engine.connect() as conn:
            ...
            housing_data_exists, address_mappings_exists, unique_addresses_exists = result
        
        if not (housing_data_exists and address_mappings_exists and unique_addresses_exists):
            missing_tables = []
        ...
        if record_count == 0:
            print("No geocoded properties found with sale prices. Please check your data or geocoding results.")
            sys.exit(1)
        ...
        for idx, row in properties_df.iterrows():
            ...
            display_address = row['address_standardized'] if pd.notna(row['address_standardized']) else row['property_address']
            
            # Format numeric values with proper handling for NaN values
            bedrooms_str = str(int(row['bedrooms'])) if pd.notna(row['bedrooms']) else 'N/A'
            ...
            # Format year built
            year_built_str = str(int(row['year_built'])) if pd.notna(row['year_built']) else 'N/A'
            
            # Format acreage
            acreage_str = f"{row['acreage']:.2f}" if pd.notna(row['acreage']) else 'N/A'
            
            # Format total value - this fixes the error
            total_value_str = f"${row['total_value']:,.0f}" if pd.notna(row['total_value']) else 'N/A'
