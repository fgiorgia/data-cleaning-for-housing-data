...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("geocoding_cli.log"), logging.StreamHandler()],
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Geocode addresses from the unique_addresses table"
    )
...
def show_stats(service: GeocodingService) -> None:
    ...
    try:
        with service.engine.connect() as conn:
            ...
            total_count: int = (
                conn.execute(text("SELECT COUNT(*) FROM unique_addresses")).scalar()
                or 0
            )
            geocoded_count: int = conn.execute(text("""
            ...
            geocoded_percentage = (
                (geocoded_count / total_count * 100) if total_count > 0 else 0
            )
...
def geocode_addresses(args: argparse.Namespace) -> None:
    ...
    for i in range(0, total, args.batch_size):
        ...
        for address_id in batch:
            try:
                ...
                logger.error(
                    f"Error fetching address_standardized info for ID {address_id}: {e}"
                )
            ...
            try:
                ...
                if result.get("from_cache"):
                    print(
                        f"    Found in cache: {result.get('source')} - {result.get('status')}"
                    )
                else:
        ...
        print(
            f"Batch time: {batch_time:.1f} seconds ({batch_time / len(batch):.1f} seconds per address)"
        )
    ...
    if processed > 0:  # Avoid division by zero
        print(
            f"Successfully geocoded: {successful} ({successful / processed * 100:.1f}% success rate)"
        )
    else:
